...
class SechTest(unittest.TestCase):    
    ...
    def tearDown(self):
        pass
    def test_wavelength_meters(self):   
        self.assertAlmostEqual(self.sech.wavelength_m[int(self.sech.n/2)], 1064.*1e-9)
    def test_wavelength_nm(self): 
        self.assertAlmostEqual(self.sech.wavelength_nm[int(self.sech.n/2)], 1064.)
    def test_frequency_Hz(self):        
        self.assertAlmostEqual(self.sech.W_Hz[int(self.sech.n/2)] /\
            ( 2*np.pi*299792458/(1064.*1e-9)), 1.0)
    def test_frequency_THz(self):  
        self.assertAlmostEqual(self.sech.W_THz[int(self.sech.n/2)] /\
            ( 1e-12*2*np.pi*299792458/(1064.*1e-9)), 1.0)
    def test_npts(self):
        self.assertEqual(self.sech.n, self.TEST_PTS)
        self.assertEqual(self.sech.n,   self.TEST_PTS)
    def test_timewindow(self):
        self.assertAlmostEqual(self.sech.time_window_ps, 12.345)
        self.assertAlmostEqual(self.sech.time_window_s, 12.345e-12)
    def test_timeaxis(self):
        self.assertAlmostEqual(self.sech.T_ps[-1] - self.sech.T_ps[0], 12.345,1)
        self.assertAlmostEqual(self.sech.T_s[-1] - self.sech.T_s[0], 12.345e-12,1) 
