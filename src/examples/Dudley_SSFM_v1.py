...
# Match propagation axis length to available field snapshots for plotting.
y_plot = y[:zW.shape[0]]
...
plt.figure()
plt.subplot(121)
plt.pcolormesh(xW, y_plot, zW, shading='nearest', vmin = mlIW - 40.0, vmax = mlIW)
...
plt.subplot(122)
plt.pcolormesh(xT, y_plot, zT, shading='nearest', vmin = mlIT - 40.0, vmax = mlIT)
plt.autoscale(tight=True)
plt.xlabel('Delay (ps)')
plt.ylabel('Distance (m)')
